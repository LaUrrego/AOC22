with open("day6/input/input.txt") as file:
    ...
    for position in range(len(clean_line)):
        working_sequence = [clean_line[position:position + 14]]
        counter = 0
        ...
        if counter == 14:
            print("14 unique after: ", position + 5)
            print("Current position in list is: ", position + 1)
            collection.append([f"current position with unique: {position + 1}, so after {position + 14}", f"sequence is {working_sequence}"])
